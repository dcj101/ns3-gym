 ...
 *
这段代码是一个用于实现OpenAI Gym数据容器的C++类库。其中包括了三个不同的数据容器类：
OpenGymDiscreteContainer、OpenGymBoxContainer和OpenGymTupleContainer。OpenGymDiscreteContainer是一个离散数据容器，
OpenGymBoxContainer是一个连续数据容器，OpenGymTupleContainer是一个元组数据容器。这些容器类都继承自OpenGymDataContainer类，
该类是一个抽象类，定义了一些虚函数，需要在子类中实现。这些容器类都可以将数据打包成protobuf消息，以便在不同的进程之间传递。

要回传一个矩阵，您可以使用OpenGymBoxContainer类。该类是一个连续数据容器，可以存储任何类型的向量。它还有一个GetShape()方法，
返回存储在容器中的数据的形状，以无符号整数向量的形式。要创建一个矩阵，您可以使用一个形状向量初始化OpenGymBoxContainer的实例，
其中第一个元素是行数，第二个元素是列数。例如，要创建一个2x3的浮点数矩阵，您可以使用以下代码：

std::vector<uint32_t> shape = {2, 3};
OpenGymBoxContainer<float> matrix(shape);

然后，您可以使用AddValue()方法向矩阵添加数据，并使用GetValue()方法检索数据。
